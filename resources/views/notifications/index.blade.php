            ...
            <li class="p-4 rounded-xl mb-2 transition hover:bg-gray-50 
                ...
                <div class="flex justify-between items-start">
                    <div class="flex-1">
                        ...
                        @if(!empty($note->data['technician_name']))
                            <p class="text-xs text-gray-500">
                                🧑‍🔧 Technician: {{ $note->data['technician_name'] }}
                            </p>
                        @endif
                        ...
                        @if(!empty($note->data['service_request_id']))
                            <a href="{{ route('service_requests.show', $note->data['service_request_id']) }}" 
                    ...
                    <div class="text-right ml-4">
                        ...
                        @if(!$isRead)
                            <form method="POST" action="{{ route('notifications.markRead', $note->id) }}" class="mt-2">
                                @csrf
                                <button type="submit"
