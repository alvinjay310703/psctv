...
<div class="bg-white rounded-2xl shadow-md p-6">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <div class="p-4 bg-blue-50 rounded-xl text-center shadow-sm">
            ...
            <p class="text-lg font-bold text-blue-700">₱{{ number_format($customer->total_paid, 2) }}</p>
        </div>
        <div class="p-4 bg-red-50 rounded-xl text-center shadow-sm">
            <p class="text-sm text-gray-600">Outstanding Balance</p>
            <p class="text-lg font-bold text-red-600">₱{{ number_format($customer->outstanding_balance, 2) }}</p>
        </div>
        ...
        <tbody>
            @forelse($customer->subscriptions as $sub)
                <tr class="hover:bg-gray-50">
                    ...
                    {{-- Start Date --}}
                    <td class="p-2 border">
                        {{ $sub->start_date ? \Carbon\Carbon::parse($sub->start_date)->format('M d, Y') : '—' }}
                    </td>

                    {{-- End Date --}}
                    <td class="p-2 border">
                        {{ $sub->end_date ? \Carbon\Carbon::parse($sub->end_date)->format('M d, Y') : 'Ongoing' }}
                    </td>

                    {{-- Monthly Fee --}}
                    <td class="p-2 border">₱{{ number_format($sub->package->price ?? 0, 2) }}</td>
                ...
                <tbody>
                    @forelse($customer->serviceRequests as $req)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="p-2 border">{{ ucfirst($req->type) }}</td>
                            <td class="p-2 border">{{ $req->created_at->format('M d, Y') }}</td>
                            <td class="p-2 border">{{ $req->technician->user->name ?? 'Unassigned' }}</td>
                            <td class="p-2 border">
                                <span class="px-2 py-1 rounded-full text-xs font-semibold 
